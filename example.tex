...
% Use the Tufte theme
% Options:
%   - Color: dark (default), light, solarizedlight, solarizeddark, solarizedosaka,
%            nord, gruvboxlight, gruvboxdark, autumn
%   - Font: serif (default) or sans
%   - Layout: nosectionheader (hide section from header), navontop (show navigation above title)
%   - TOC: nosub (hide subsections in table of contents)
%
% Examples:
%   \usetheme[solarizedosaka]{Tufte}                    % Just color scheme
%   \usetheme[nord, sans]{Tufte}                        % Color + font
%   \usetheme[dark, navontop]{Tufte}                    % Show navigation squares above title
%   \usetheme[gruvboxdark, sans, nosectionheader]{Tufte} % Multiple options
\usetheme[solarizeddark, sans, nosub, navontop, nosectionheader]{Tufte}
...
\begin{frame}{TikZ Diagrams}
  ...
  \begin{tikzpicture}[scale=0.8]
    % Nodes
    \node[circle,draw=tuftetext,fill=tuftegray!20,text=tuftetext,minimum size=1cm] (A) at (0,0) {A};
    \node[circle,draw=tuftetext,fill=tuftegray!20,text=tuftetext,minimum size=1cm] (B) at (3,1) {B};
    \node[circle,draw=tuftetext,fill=tuftegray!20,text=tuftetext,minimum size=1cm] (C) at (3,-1) {C};
    \node[circle,draw=tufteaccent,fill=tufteaccent!15,text=tuftetext,minimum size=1cm] (D) at (6,0) {D};

    % Edges
    \draw[->,thick,color=tuftetext] (A) -- (B) node[midway,above,text=tuftetext] {5};
    \draw[->,thick,color=tuftetext] (A) -- (C) node[midway,below,text=tuftetext] {3};
    \draw[->,thick,color=tuftetext] (B) -- (D) node[midway,above,text=tuftetext] {2};
    \draw[->,thick,color=tuftetext] (C) -- (D) node[midway,below,text=tuftetext] {4};
    \draw[->,thick,color=tufteaccent] (B) -- (C) node[midway,right,text=tufteaccent] {1};
  \end{tikzpicture}

  \vspace{0.5em}

  \small Weighted directed graph using themed colors (tuftetext, tufteaccent, tuftegray)
\end{frame}
...
\begin{frame}{PGFPlots: Tufte Style}
  Simple activation with \texttt{tufte style} (no grid, legend outside):
...
\begin{frame}{Theme Options Reference}
  ...
  \begin{tufteframe}{Layout Options}
  ...
  \textbf{Usage:} \texttt{\textbackslash usetheme[nord,sans,navontop]\{Tufte\}}
\end{frame}
...
\begin{frame}{Custom Footer Text}
  ...
  \begin{tufteframe}{footertext Command}
